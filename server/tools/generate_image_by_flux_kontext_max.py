...
class GenerateImageByFluxKontextMaxInputSchema(BaseModel):
    ...
    model: str = Field(
        description="Required. The model to use for image generation, e.g. 'flux/flux-kontext-max'"
    )
    tool_call_id: Annotated[str, InjectedToolCallId]


@tool("generate_image_by_flux_kontext_max",
      description="Generate an image by Flux Kontext Max model using text prompt. This model does NOT support input images for reference or editing. Use this model for high-quality image generation with Flux's advanced AI. Supports multiple providers with automatic fallback.",
